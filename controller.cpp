...
void Controller::drawField(QPainter &painter, int xOffset) {
    ...
    for (int i = 0; i < 10; i ++) {
        painter.drawLine(QLine(xOffset + currentLineStart, FIRST_LINE_OFFSET, xOffset+currentLineStart, FIRST_LINE_OFFSET + SCREEN_DIM));
        painter.drawLine(QLine(xOffset, FIRST_LINE_OFFSET + currentLineStart, xOffset + SCREEN_DIM, FIRST_LINE_OFFSET + currentLineStart));
